...
class Expr
{
	...
	// for code generation
	virtual gen::Constant
	loadConstant() const = 0;
	virtual gen::Value
	loadValue() const = 0;
	virtual gen::Constant
	loadConstantAddress() const;
	virtual gen::Value
	loadAddress() const = 0;
	virtual void
	condition(gen::Label trueLabel, gen::Label falseLabel) const;

	// for debugging and educational purposes
	virtual void
	print(int indent = 1) const = 0;

	// for printing error messages
	virtual void
	printFlat(std::ostream &out, int prec) const = 0;

	// get value from const expressions with integer type
	gen::ConstantInt
	getConstantInt() const;
	std::int64_t
	getSignedIntValue() const;
	std::uint64_t
