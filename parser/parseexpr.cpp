...
parseAssignmentExpression()
{
    ...
    while (
      token.kind == TokenKind::EQUAL || token.kind == TokenKind::PLUS_EQUAL ||
      ...
      token.kind == TokenKind::AND_EQUAL || token.kind == TokenKind::OR_EQUAL ||
...
parseBinary(int prec)
{
    ...
    for (int p = tokenKindPrec(token.kind); p >= prec; --p) {
        while (tokenKindPrec(token.kind) == p) {
            ...
            expr =
              BinaryExpr::create(op, std::move(expr), std::move(right), opLoc);
        }
    }
    return expr;
}
...
parsePrefix()
{
    ...
    switch (token.kind) {
        ...
        case TokenKind::AND:
            getToken();
            return UnaryExpr::create(UnaryExpr::ADDRESS, parsePrefix(),
                                     tok.loc);
        case TokenKind::ASTERISK:
            getToken();
        ...
        case TokenKind::PLUS2:
            getToken();
            return UnaryExpr::create(UnaryExpr::PREFIX_INC, parsePrefix(),
                                     tok.loc);
        case TokenKind::MINUS2:
            getToken();
            return UnaryExpr::create(UnaryExpr::PREFIX_DEC, parsePrefix(),
                                     tok.loc);
        case TokenKind::MINUS:
            getToken();
    ...
    switch (tok.kind) {
        ...
        case TokenKind::LPAREN:
            ...
            // function call
            {
                ...
                getToken();
                expr =
                  CallExpr::create(std::move(expr), std::move(arg), tok.loc);
                return parsePostfix(std::move(expr));
            }
        ...
        case TokenKind::ARROW:
            ...
            if (token.kind == TokenKind::IDENTIFIER) {
                ...
                return parsePostfix(UnaryExpr::create(
                  UnaryExpr::ARROW_DEREF, std::move(expr), tok.loc));
            }
        case TokenKind::PLUS2:
            getToken();
