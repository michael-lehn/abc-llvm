...
BinaryExpr::create(Kind kind, ExprPtr &&left, ExprPtr &&right, lexer::Loc loc)
{
    ...
    assert(right);
    auto promotion =
        promotion::binary(kind, std::move(left), std::move(right), &loc);
    auto p = new BinaryExpr{kind, std::move(std::get<0>(promotion)),
...
BinaryExpr::isConst() const
{
	...
	auto diff = gen::pointerConstantDifference(refType, left->loadValue(),
	                                           right->loadValue());
...
BinaryExpr::loadConstant() const
{
	...
	if (left->type->isPointer() && right->type->isPointer()) {
	    ...
	    assert(Type::equals(left->type->refType(), right->type->refType()));
	    auto refType = left->type->refType();
	    auto diff = gen::pointerConstantDifference(
	        refType, left->loadValue(), right->loadValue());
    ...
    case BITWISE_RIGHT_SHIFT:
	return gen::instruction(getGenInstructionOp(kind, type),
	                        left->loadConstant(), right->loadConstant());
    case LESS:
    case LESS_EQUAL:
    case GREATER:
    case GREATER_EQUAL:
...
BinaryExpr::handleArithmetricOperation(Kind kind) const
{
	...
	if (type->isPointer()) {
	    ...
	    return gen::pointerIncrement(left->type->refType(),
	                                 left->loadValue(), right->loadValue());
	} else {
	    return gen::instruction(getGenInstructionOp(kind, type),
	                            left->loadValue(), right->loadValue());
	}
	...
	if (kind == SUB && left->type->isPointer()) {
	    ...
	    return gen::pointerDifference(
	        left->type->refType(), left->loadValue(), right->loadValue());
	} else {
	    return gen::instruction(getGenInstructionOp(kind, type),
	                            left->loadValue(), right->loadValue());
	}
...
BinaryExpr::loadValue() const
{
	...
	return gen::phi(gen::getTrue(), trueLabel, gen::getFalse(), falseLabel,
...
BinaryExpr::loadConstantAddress() const
{
	...
	auto offset = right->getUnsignedIntValue();
	return gen::pointerIncrement(left->type->refType(),
	                             left->loadConstantAddress(), offset);
    } else {
	auto offset = right->getUnsignedIntValue();
	return gen::pointerIncrement(left->type->refType(),
	                             left->loadConstant(), offset);
...
BinaryExpr::loadAddress() const
{
    ...
    assert(kind == INDEX);
    if (left->type->isArray()) {
	return gen::pointerIncrement(left->type->refType(), left->loadAddress(),
	                             right->loadValue());
    } else {
	return gen::pointerIncrement(left->type->refType(), left->loadValue(),
	                             right->loadValue());
    }
}
...
BinaryExpr::condition(gen::Label trueLabel, gen::Label falseLabel) const
{
	...
	auto cond = gen::instruction(op, left->loadValue(), right->loadValue());
...
{
    switch (kind) {
    case abc::BinaryExpr::ADD:
	return "+";
    case abc::BinaryExpr::ASSIGN:
	return "=";
    case abc::BinaryExpr::ADD_ASSIGN:
	return "+=";
    case abc::BinaryExpr::SUB_ASSIGN:
	return "-=";
    case abc::BinaryExpr::MUL_ASSIGN:
	return "*=";
    case abc::BinaryExpr::DIV_ASSIGN:
	return "/=";
    case abc::BinaryExpr::MOD_ASSIGN:
	return "%=";
    case abc::BinaryExpr::BITWISE_AND_ASSIGN:
	return "&=";
    case abc::BinaryExpr::BITWISE_OR_ASSIGN:
	return "|=";
    case abc::BinaryExpr::BITWISE_XOR_ASSIGN:
	return "^=";
    case abc::BinaryExpr::BITWISE_LEFT_SHIFT_ASSIGN:
	return "<<=";
    case abc::BinaryExpr::BITWISE_RIGHT_SHIFT_ASSIGN:
	return ">>=";
    case abc::BinaryExpr::EQUAL:
	return "==";
    case abc::BinaryExpr::NOT_EQUAL:
	return "!=";
    case abc::BinaryExpr::GREATER:
	return ">";
    case abc::BinaryExpr::GREATER_EQUAL:
	return ">=";
    case abc::BinaryExpr::LESS:
	return "<";
    case abc::BinaryExpr::LESS_EQUAL:
	return "<=";
    case abc::BinaryExpr::LOGICAL_AND:
	return "&&";
    case abc::BinaryExpr::LOGICAL_OR:
	return "||";
    case abc::BinaryExpr::SUB:
	return "-";
    case abc::BinaryExpr::MUL:
	return "*";
    case abc::BinaryExpr::DIV:
	return "/";
    case abc::BinaryExpr::MOD:
	return "%";
    case abc::BinaryExpr::BITWISE_AND:
	return "&";
    case abc::BinaryExpr::BITWISE_OR:
	return "|";
    case abc::BinaryExpr::BITWISE_XOR:
	return "^";
    case abc::BinaryExpr::BITWISE_LEFT_SHIFT:
	return "<<";
    case abc::BinaryExpr::BITWISE_RIGHT_SHIFT:
	return ">>";
    default:
	return "?? (binary)";
    }
