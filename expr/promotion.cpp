...
call(ExprPtr &&fn, std::vector<ExprPtr> &&arg, lexer::Loc *loc)
{
	...
	if (loc) {
	    ...
	    error::out() << fn->loc << error::setColor(error::BOLD_RED) << ": "
          ...
          lexer::Loc *loc)
{
	...
	error::out() << error::setColor(error::BOLD) << *loc << ": "
	             ...
	             << "operator can not be applied to operand '" << left
	             << "' of type '" << left->type << "' and  operand '"
	             << right << "' of type '" << right->type << "'\n"
	             << error::setColor(error::NORMAL);
	error::fatal();
    }
          ...
          lexer::Loc *loc)
{
	...
	if (!Type::assignable(left->type)) {
	    error::out() << error::setColor(error::BOLD) << left->loc << ": "
	                 << error::setColor(error::BOLD_RED)
	                 << "error: " << error::setColor(error::BOLD)
	                 << ": assignment of read-only variable '" << left
	                 << "'\n"
	                 << error::setColor(error::NORMAL);
          ...
          lexer::Loc *loc)
{
	...
	if (left->type->isPointer() && right->type->isPointer()) {
	    type = IntegerType::createSigned(64);
	    newLeftType = newRightType = Type::common(left->type, right->type);
